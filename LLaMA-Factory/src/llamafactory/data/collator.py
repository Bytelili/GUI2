...
class SFTDataCollatorWith4DAttentionMask(MultiModalDataCollatorForSeq2Seq):
    ...
    def __call__(self, features: list[dict[str, Any]]) -> dict[str, "torch.Tensor"]:
        ...
        model_features = [
            {key: value for key, value in feature.items() if key != "listwise_weight"}
            for feature in features
        ]
...
class PairwiseDataCollatorWithPadding(MultiModalDataCollatorForSeq2Seq):
    ...
    def __call__(self, features: list[dict[str, Any]]) -> dict[str, "torch.Tensor"]:
        ...
        preference_weights = torch.tensor(
            [float(feature.get("preference_weight", 1.0)) for feature in features], dtype=torch.float32
        )
        preference_targets = torch.tensor(
            [float(feature.get("preference_target", 1.0)) for feature in features], dtype=torch.float32
        )
        ...
        batch["preference_weight"] = preference_weights
        batch["preference_target"] = preference_targets
        return batch
