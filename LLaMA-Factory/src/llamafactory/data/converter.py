...
class AlpacaDatasetConverter(DatasetConverter):
    def __call__(self, example: dict[str, Any]) -> dict[str, Any]:
        ...
        output = {
            ...
            "_listwise_weight": example.get(self.dataset_attr.listwise_weight, 1.0)
            if self.dataset_attr.listwise_weight
            else 1.0,
            "_preference_weight": example.get(self.dataset_attr.preference_weight, 1.0)
            if self.dataset_attr.preference_weight
            else 1.0,
            "_preference_target": example.get(self.dataset_attr.preference_target, 1.0)
            if self.dataset_attr.preference_target
            else 1.0,
        }
        return output
