...
def normalized_weighted_mean(losses: "torch.Tensor", weights: "torch.Tensor") -> "torch.Tensor":
    r"""Compute a stable normalized weighted mean for PAPO preference losses."""
    weights = weights.to(device=losses.device, dtype=losses.dtype).clamp_min(0.0)
    return (losses * weights).sum() / weights.sum().clamp_min(torch.finfo(losses.dtype).eps)


def papo_soft_pair_loss(
    ...
    r"""Compute PAPO soft-label BCE over policy-to-reference pairwise log-ratios."""
    model_logits = (policy_chosen_logps - reference_chosen_logps) - (
        policy_rejected_logps - reference_rejected_logps
    )
    targets = targets.to(device=model_logits.device, dtype=model_logits.dtype).clamp(0.0, 1.0)
    return F.binary_cross_entropy_with_logits(model_logits, targets, reduction="none")
...
class CustomDPOTrainer(DPOTrainer):
    def __init__(
        ...
        **kwargs,
        ...
        self.papo_weighted = finetuning_args.pref_loss == "papo"
        self.loss_type = "sigmoid" if self.papo_weighted else finetuning_args.pref_loss
    ...
    def get_batch_loss_metrics(
        ...
        metrics = {}
        preference_weight = batch.pop("preference_weight", None)
        preference_target = batch.pop("preference_target", None)
        ...
        if self.papo_weighted and preference_target is not None:
            losses = papo_soft_pair_loss(
                policy_chosen_logps,
                policy_rejected_logps,
                reference_chosen_logps,
                reference_rejected_logps,
        ...
        if self.papo_weighted:
            if preference_weight is None:
                preference_weight = torch.ones_like(losses)
            else:
                preference_weight = preference_weight.to(device=losses.device, dtype=losses.dtype)

            preference_weight = preference_weight.clamp_min(0.0)
            metrics[f"{prefix}papo/weight_mean"] = preference_weight.mean().item()
            if preference_target is not None:
                metrics[f"{prefix}papo/target_mean"] = preference_target.float().mean().item()
            weighted_loss = normalized_weighted_mean(losses, preference_weight)
            metrics[f"{prefix}papo/weighted_loss"] = weighted_loss.item()
