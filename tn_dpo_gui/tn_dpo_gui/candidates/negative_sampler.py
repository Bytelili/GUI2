...
def default_negatives(preferred_type_text: str | None = None) -> list[Action]:
    return [
        ...
        Action(action_type="type", target="Search box", text=preferred_type_text or None),
    ]


def sample_negative_actions(
    current_action,
    extra_pool: list[Action] | None = None,
    max_negatives: int = 4,
    preferred_type_text: str | None = None,
) -> list[Action]:
    current_key = normalize_action(current_action).normalized_key()
    pool = deduplicate_actions([*(extra_pool or []), *default_negatives(preferred_type_text)])
